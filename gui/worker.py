...
class EjecutorPSINet(QObject):
    ...
    def _eliminar_archivos(
        ...
        for ruta_texto in dict.fromkeys(rutas):
            ...
            try:
                ...
                self.mensaje.emit(
                    f"No se pudo eliminar {tipo} {ruta.name}: {error}"
                )
    ...
    def _guardar_y_descargar(
        ...
        if destino_pdf is None:
            raise RuntimeError(
                "PSINet guardó la mantención, pero CCTVFlow no pudo "
                ...
                with iniciar_navegador(headless=False) as page:
                    ...
                    for indice, item in enumerate(self.plan, start=1):
                        ...
                        try:
                            ...
                            cantidad_fotos_esperadas = len(
                                evidencia["fotos"]
                            )

                            if not self.modo_automatico:
                                # En el modo de respaldo se cargan las dos ART
                            ...
                            self.mensaje.emit(
                                ...
                                f"{item.camara}\n"
                                f"{error}\n"
                                "PSINet alcanzó a guardar esta mantención. "
                    ...
                    if (
                        ...
                        fotos_art = [
                            foto
                            for sector in {
                                item.sector for item in self.plan
                            }
